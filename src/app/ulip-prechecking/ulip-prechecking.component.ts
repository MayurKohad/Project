...
@Component({
  selector: 'app-ulip-prechecking',
  templateUrl: './ulip-prechecking.component.html',
  styleUrls: ['./ulip-prechecking.component.scss']
})
export class UlipPrecheckingComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.edit_data = {};
    this.displayedColumns = ['POL_ID', 'CIA_PRCES_DT', 'CIA_EFF_DT', 'CIA_TYP_CD', 
    'CIA_REASN_CD', 'FUND_DETAILS', 'FUND_VALUE', 'MONEY_IN_TRACKING'];
    this.setTableData();
...
return data[type]
  ...
  filterDataByPolicyId(id) {
    this.filteredData = this.transaction_list.filter(function (data) {
      console.log('filterDataByPolicyId >>>>', data,id)
